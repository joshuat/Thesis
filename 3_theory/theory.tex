...
\begin{tikzpicture}[scale=2]
    \begin{scope}[thick, decoration={snake,amplitude=.4mm,
      ...
      \draw (0, 0) node[left=0.4cm] {$\ket{g}$}
        -- (2, 0);
      \draw (0, 1.5)  node[left=0.4cm] {$\ket{e}$}
        -- (2, 1.5);
...
\begin{tikzpicture}[scale=2]
    ...
    \filldraw[fill=blue] (0, 1.9) circle (0.05) node[above] {\tiny $e^{\mbox{-}}$};
    \draw [color=black, dotted] (-1.4, 1.9) -- (1.4, 1.9);
    \draw (-0.8, 2.2) node {\tiny No electric field};
    \draw [dashed, color=gray] (-1.4, 2) node[color=black, left] {\tiny $V=0$} -- (1.4, 2);

    \draw[domain=-1.4:-0.15, color=black] plot (\x, {-1/(50*\x*\x)+2});
    \draw[domain=0.15:1.4, color=black] plot (\x, {-1/(50*\x*\x)+2}) node[right] {\tiny Atomic Potential};

    \filldraw[fill=blue] (0, -0.1) circle (0.05) node[above] {\tiny $e^{\mbox{-}}$};
    \draw [color=black, dotted] (-1.4, -0.1) -- (1.4, -0.1);
    \draw [color=blue, above=4, ->] (0.2, -0.1) -- (1.4, -0.1);
    \draw [color=red, ->] (-0.5, -1.2) -- (0.5, -1.2) node [right] {\tiny $\vec{E}$};
    \draw [dashed, color=gray] (-1.4, 0) node[color=black, left] {\tiny $V=0$} -- (1.4, 0);

    \draw[domain=-1.4:-0.143, color=black] plot (\x, {-1/(50*\x*\x)-0.3*\x});
    \draw[domain=0.15:1.4, color=black] plot (\x, {-1/(50*\x*\x)-0.3*\x}) node[right] {\tiny Distorted Atomic Potential};

    \end{scope}
\end{tikzpicture}
\caption[Title]{The top diagram dipicts an atom excited to a Rydberg state without the presence of an electric field. The lower diagram show the same atom in the presence of an electric field. The electric field distorts the atomic potential in such a way as to allow the atom to become ionised.}
\end{figure}
