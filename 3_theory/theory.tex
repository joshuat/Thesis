...
As the probe laser traverses through the atoms cloud it obeys Beer's law,
\begin{equation}\label{eq:beers_law}
I(x, y) = I_0(x, y)e^{-D(x, y)}
\end{equation}
where D(x, y) is the optical depth along the probe beam's axis at a given position (x, y) in the plane perpendicular to the probe beam. The optical density is given by
...
\begin{equation}
N = \frac{A}{\sigma} \sum D(x, y)
\end{equation}
where $A$ is the scaled area of the detector.

The relavant cross-section, $\sigma$ can be obtained from Steck\cite{} {\color{red} reword this, cite Steck, finish getting $\sigma$ and describe how to get scale factor}.
