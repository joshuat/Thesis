...
If $I_0$ is an image of the probe beam taken while there are no atoms present and $I_{abs}$ is an image of the probe beam with the atoms present then the transmission function for the atoms, $T$, can be calculated with
...
Absorption imaging can be used to determine the number of atoms in a given trap.

{\color{red} work in progress...}

The total power of the laser is distributed across the `true' background image. This provides us with a counts $\rightarrow$ power/intensiity ratio (assume linear?).

Counts in `shadow' image $\rightarrow$ intensity loss $\rightarrow$ number of photons absorbed.
Number of photons absorbed / chance of absorption $\rightarrow$ number of atoms

Need to know, power of beam, exposure (the real one ie. with stingray offset, what about camera exposure/laser exposure?) to get intensity. Also need to figure out what the probability of absorption is. Some fluorescence will reach the camera. Is this an issue?
